...
void main() {
  testWidgets("new streams clear player destinations and preserve the nested tab stack", (
    tester,
  ) async {
    ...
    for (final (destination, nextStream) in [
      ...
      unawaited(
        openStreamPlayer(
          tester.element(find.byKey(ValueKey(destination))),
          builder: (_) => page(nextStream),
        ),
      );
      await tester.pumpAndSettle();
      expect(find.byKey(ValueKey(destination), skipOffstage: false), findsNothing);
